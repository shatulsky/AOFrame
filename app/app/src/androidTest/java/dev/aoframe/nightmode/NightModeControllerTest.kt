 ...
 * scheduling logic, using injected clock/root-shell-action seams (added
 * specifically so this can be tested - see the class's own comment) to
 * drive checkOnce() directly instead of waiting on the real 30s loop/wall
 * clock. NightModeTiming's own pure logic (shouldTrigger/isWithinWindow)
 * is already covered by NightModeTimingTest - these tests focus on the
 * controller's own state (once-per-day dedup, config-disabled short
 * circuit, malformed-config resilience).
 */
class NightModeControllerTest {
    ...
    private fun controllerAt(sleepTime: String, wakeTime: String, minute: Int) = NightModeController(
        context = context,
        scope = scope,
        today = { day },
        nowMinutes = { minute },
    ...
    fun checkOnceTriggersWakeAtTheConfiguredMinute() {
        val controller = controllerAt(sleepTime = "01:00", wakeTime = "08:00", minute = NightModeTiming.parseMinutesOfDay("08:00"))
